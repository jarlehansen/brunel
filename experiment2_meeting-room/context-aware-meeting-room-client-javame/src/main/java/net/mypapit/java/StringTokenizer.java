 ...
 * tokens. More information about this class is available from <a target="_top"
 * href=
 ...
 * </code></blockquote>
 * <p>
 * prints the following output: <blockquote> (<br>
 * aaa<br>
 **<br>
 * (<br>
 * b<br>
 ...
 * </code></blockquote>
 * <p>
 * prints the following output: <blockquote> x<br>
 * a<br>
 * b<br>
 * c </blockquote>
 * <p>
 ...
 * However, after these changes the output of the above code was: <blockquote> x
 * <br>
 * =a<br>
 * b<br>
 * c </blockquote>
 ...
 * should be modified as: <blockquote><code>
 * String s = "x=a,b,c";<br>
...
public class StringTokenizer implements java.util.Enumeration {
	...
	/**
	 * The length of the text. Cached for performance. This should be set
	 * whenever the string we are working with is changed.
	 * 
	 * @since ostermillerutils 1.00.00
	 */
	...
	/**
	 * One of two variables used to maintain state through the tokenizing
	 * process.
	 * <P>
	 * Represents the position at which we should start looking for the next
	 * token(the position of the character immediately following the end of the
	 * last token, or 0 to start), or -1 if the entire string has been examined.
	 * 
	 * @since ostermillerutils 1.00.00
	 */
	 ...
	 * process.
	 * <p>
	 * true if and only if is found that an empty token should be returned or if
	 * empty token was the last thing returned.
	 * <p>
	...
	/**
	 * Stores the value of the delimiter character with the highest value. It is
	 * used to optimize the detection of delimiter characters. The common case
	 * will be that the int values of delimiters will be less than that of most
	 * characters in the string (, or space less than any letter for example).
	 * Given this, we can check easily check to see if a character is not a
	 * delimiter by comparing it to the max delimiter. If it is greater than the
	 * max delimiter, then it is no a delimiter otherwise we have to do some
	 * more in depth analysis. (ie search the delimiter string.) This will
	 * reduce the running time of the algorithm not to depend on the length of
	 * the delimiter string for the common case.
	 * 
	 * @since ostermillerutils 1.00.00
	 */
	...
	/**
	 * Whether empty tokens should be returned. ie if "" should be returned when
	 * text starts with a delim, has two delims next to each other, or ends with
	 * a delim.
	 * 
	 * @since ostermillerutils 1.00.00
	 */
	...
	/**
	 * Indicates at which position the delimiters last changed. This will effect
	 * how null tokens are returned. Any time that delimiters are changed, the
	 * string will be treated as if it is being parsed from position zero, ie,
	 * null strings are possible at the very beginning.
	 * 
	 * @since ostermillerutils 1.00.00
	 */
	...
	/**
	 * A cache of the token count. This variable should be -1 if the token have
	 * not yet been counted. It should be greater than or equal to zero if the
	 ...
	 *            the nontoken delimiters, i.e. the delimiters that only
	 *            separate tokens and are not returned as separate tokens.
	 * @param tokenDelims
	 *            the token delimiters, i.e. delimiters that both separate
	 *            tokens, and are themselves returned as tokens.
	...
	/**
	 * Constructs a string tokenizer for the specified string. Both token and
	 * nontoken delimiters are specified and whether or not empty tokens are
	 * returned is specified.
	 * <p>
	 * Empty tokens are tokens that are between consecutive delimiters.
	 * <p>
	 * It is a primary constructor (i.e. all other constructors are defined in
	 * terms of it.)
	 * <p>
	 * The current position is set at the beginning of the string.
	 * 
	 ...
	 *            true if empty tokens may be returned; false otherwise.
	 * @throws NullPointerException
	 *             if text is null.
	 * 
	 * @since ostermillerutils 1.00.00
	 ...
	 *            flag indicating whether the second parameter specifies token
	 *            or nontoken delimiters: <code>false</code> -- the second
	 *            parameter specifies nontoken delimiters, the set of token
	 *            delimiters is empty; <code>true</code> -- the second parameter
	 *            specifies token delimiters, the set of nontoken delimiters is
	 *            empty.
	 * @throws NullPointerException
	 *             if text is null.
	 * 
	 ...
	 * the <code>nontokenDelims</code> argument are the delimiters for
	 ...
	 * uses " \t\n\r\f" as a delimiter set of nontoken delimiters, and an empty
	 * token delimiter set.
	 * <p>
	 ...
	 * <p>
	 * This is useful when for StringTokenizer re-use so that new string
	 * tokenizers do no have to be created for each string you want to
	 * tokenizer.
	 * <p>
	...
	/**
	 * Set the delimiters for this StringTokenizer. The position must be
	 * initialized before this method is used. (setText does this and it is
	 * called from the constructor)
	 * 
	 * @param nontokenDelims
	 *            delimiters that should not be returned as tokens.
	 * @param tokenDelims
	 *            delimiters that should be returned as tokens.
	 * 
	 * @since ostermillerutils 1.00.00
	 */
	 ...
	 * 
	 * @param delims
	 *            the new set of nontoken delimiters (the set of token
	 *            delimiters will be empty).
	 * 
	 * @since ostermillerutils 1.00.00
	 */
	 ...
	 *            nontoken delimiters: false -- the first parameter specifies
	 *            nontoken delimiters, the set of token delimiters is empty;
	 *            true -- the first parameter specifies token delimiters, the
	 *            set of nontoken delimiters is empty.
	 * 
	 * @since ostermillerutils 1.00.00
	 */
	 ...
	 * 
	 * @param nontokenDelims
	 *            the new set of nontoken delimiters.
	 * @param tokenDelims
	 *            the new set of token delimiters.
	 * 
	 * @since ostermillerutils 1.00.00
	 */
	 ...
	 * Calculates the number of times that this tokenizer's
	 * <code>nextToken</code> method can be called before it generates an
	 * exception using the given set of (nontoken) delimiters. The delimiters
	 * given will be used for future calls to nextToken() unless new delimiters
	 * are given. The current position is not advanced.
	 * 
	 * @param delims
	 *            the new set of nontoken delimiters (the set of token
	 *            delimiters will be empty).
	 ...
	 * Calculates the number of times that this tokenizer's
	 * <code>nextToken</code> method can be called before it generates an
	 * exception using the given set of delimiters. The delimiters given will be
	 * used for future calls to nextToken() unless new delimiters are given. The
	...
	/**
	 * Advances the state of the tokenizer to the next token or delimiter. This
	 * method only modifies the class variables position, and emptyReturned. The
	 * type of token that should be emitted can be deduced by examining the
	 * changes to these two variables. If there are no more tokens, the state of
	 * these variables does not change at all.
	 * 
	 * @return true if we are at a juncture at which a token may be emitted,
	 *         false otherwise.
	 * 
	 * @since ostermillerutils 1.00.00
	 */
	private boolean advancePosition() {
		...
		// is empty we need to indicate that there is an empty token at the
		// beginning.
		...
		while (position != -1) {
			...
			if (returnEmptyTokens && !emptyReturned && position > delimsChangedPosition) {
				...
				if (c <= maxDelimChar
						&& c1 <= maxDelimChar
						&& ((nontokenDelims != null && nontokenDelims.indexOf(c) != -1) || (tokenDelims != null && tokenDelims.indexOf(c) != -1))
						&& ((nontokenDelims != null && nontokenDelims.indexOf(c1) != -1) || (tokenDelims != null && tokenDelims.indexOf(c1) != -1))) {
					emptyReturned = true;
			...
			if (c > maxDelimChar
				...
				// token found
				/*
				 * System.out.println("Token: '" + text.substring(position,
				 * (nextDelimiter == -1 ? strLength : nextDelimiter)) + "' at "
				 * + position + ".");
				 */
		...
		// handle the case that a token is at the end of the string and we
		// should
		// return empty tokens.
	 ...
	 * Then the next token (with respect to new delimiters) in the string after
	 ...
	 * First, the sets of token and nontoken delimiters are changed to be the
	 * <code>tokenDelims</code> and <code>nontokenDelims</code>, respectively;
	 * and whether or not to return empty tokens is set. Then the next token
	 * (with respect to new delimiters) in the string after the current position
	 * is returned.
	 * <p>
	 * The current position is set after the token returned.
	 * <p>
	 * The new delimiter set remains the one used for this call and empty tokens
	 * are returned in the future as they are in this call.
	 * 
	 * @param nontokenDelims
	 *            the new set of nontoken delimiters.
	 * @param tokenDelims
	 ...
	 * 
	 * @param delims
	 *            the new set of token or nontoken delimiters.
	 * @param delimsAreTokens
	 *            flag indicating whether the first parameter specifies token or
	 *            nontoken delimiters: <code>false</code> -- the first parameter
	 *            specifies nontoken delimiters, the set of token delimiters is
	 *            empty; <code>true</code> -- the first parameter specifies
	 *            token delimiters, the set of nontoken delimiters is empty.
	 * @return the next token, after switching to the new delimiter set.
	 * @throws NoSuchElementException
	 *             if there are no more tokens in this tokenizer's string.
	 * 
	...
	/**
	 * Similar to String.indexOf(int, String) but will look for any character
	 * from string rather than the entire string.
	 * 
	 * @param start
	 *            index in text at which to begin the search
	 * @return index of the first delimiter from the start index (inclusive), or
	 *         -1 if there are no more delimiters in the string
	 * 
	 * @since ostermillerutils 1.00.00
	 */
	private int indexOfNextDelimiter(int start) {
		...
		for (next = start; (c = text.charAt(next)) > maxDelimChar
				|| ((nontokenDelims == null || nontokenDelims.indexOf(c) == -1) && (tokenDelims == null || tokenDelims.indexOf(c) == -1)); next++) {
			if (next == strLength - 1) {
	...
	/**
	 * Returns the same value as the <code>hasMoreTokens()</code> method. It
	 * exists so that this class can implement the <code>Iterator</code>
	 * interface.
	 * 
	 * @return <code>true</code> if there are more tokens; <code>false</code>
	 *         otherwise.
	 ...
	 * <code>UnsupportedOperationException</code>. It exists so that this class
	 * can implement the <code>Iterator</code> interface.
	 * 
	...
	/**
	 * Set whether empty tokens should be returned from this point in in the
	 * tokenizing process onward.
	 * <P>
	 * Empty tokens occur when two delimiters are next to each other or a
	 * delimiter occurs at the beginning or end of a string. If empty tokens are
	 * set to be returned, and a comma is the non token delimiter, the following
	 * table shows how many tokens are in each string.<br>
	 * <table>
	 * <tr>
	 * <th>String
	 * <th>
	 ...
	 * </tr>
	 * <tr>
	 * <td align=right>"one,two"
	 * <td>
	 * <td>2 - normal case with no empty tokens.
	 * <td>
	 * </tr>
	 * <tr>
	 * <td align=right>"one,,three"
	 * <td>
	 * <td>3 including the empty token in the middle.
	 * <td>
	 * </tr>
	 * <tr>
	 * <td align=right>"one,"
	 * <td>
	 * <td>2 including the empty token at the end.
	 * <td>
	 * </tr>
	 * <tr>
	 * <td align=right>",two"
	 * <td>
	 * <td>2 including the empty token at the beginning.
	 * <td>
	 * </tr>
	 * <tr>
	 * <td align=right>","
	 * <td>
	 * <td>2 including the empty tokens at the beginning and the ends.
	 * <td>
	 * </tr>
	 * <tr>
	 * <td align=right>""
	 * <td>
	 * <td>1 - all strings will have at least one token if empty tokens are
	 * returned.
	 * <td>
	 * </tr>
	 * </table>
	 * 
	 * @param returnEmptyTokens
	 *            true iff empty tokens should be returned.
	 * 
	 * @since ostermillerutils 1.00.00
	 */
	...
	/**
	 * Get the the index of the character immediately following the end of the
	 * last token. This is the position at which this tokenizer will begin
	 * looking for the next token when a <code>nextToken()</code> method is
	 * invoked.
	 * 
	 * @return the current position or -1 if the entire string has been
	 *         tokenized.
	 * 
	 * @since ostermillerutils 1.00.00
	 */
	...
	/**
	 * Retrieve all of the remaining tokens in a String array. This method uses
	 * the options that are currently set for the tokenizer and will advance the
	 * state of the tokenizer such that <code>hasMoreTokens()</code> will return
	 * false.
	 * 
	...
	/**
	 * Retrieves the rest of the text as a single token. After calling this
	 * method hasMoreTokens() will always return false.
	 * 
	...
	/**
	 * Returns the same value as nextToken() but does not alter the internal
	 * state of the Tokenizer. Subsequent calls to peek() or a call to
	 * nextToken() will return the same token again.
	 * 
	 * @return the next token from this string tokenizer.
	 * @throws NoSuchElementException
	 *             if there are no more tokens in this tokenizer's string.
